    ...
    if(!feature_active)
    {
        ...
        if(choice == 1)
        {
            ...
            do
            {
                ...
                {
                    Character::change_font_colour(4);
                    std::cout << "Error: Super power number is out of range." << std::endl << std::endl;
                    Character::change_font_colour(7);
                }
            } while (super_power < 1 || super_power > super_powers.size());
            change_font_colour(6);
            super_powers_funcs[super_power - 1]();
            std::cout << super_powers[super_power - 1] << " has been used." << std::endl << std::endl;
            change_font_colour(7);
            super_powers_funcs.erase(super_powers_funcs.begin() + (super_power - 1));
...
void Wizard::print_death()
{
    ...
    type_text("My magic... it fades into darkness...");
    std::cout << std::endl;
    std::cout << R"(
                     █     █░ ██▓▒███████▒ ▄▄▄       ██▀███  ▓█████▄     ██▓  ██████    ▓█████▄ ▓█████ ▄▄▄      ▓█████▄ 
                    ▓█░ █ ░█░▓██▒▒ ▒ ▒ ▄▀░▒████▄    ▓██ ▒ ██▒▒██▀ ██▌   ▓██▒▒██    ▒    ▒██▀ ██▌▓█   ▀▒████▄    ▒██▀ ██▌
