...
extern"C" __declspec(dllexport) void generate_matrix_into_file(const std::string& file_name, int rows, int cols)
{
...
extern"C" __declspec(dllexport) matrix read_matrix_from_file(const std::string& file_name)
{
...
extern"C" __declspec(dllexport) void write_matrix_to_file(const matrix& m, const std::string& file_name) 
{
    std::ofstream out_file(file_name);
    if (!out_file) 
    {
...
extern"C" __declspec(dllexport) void CALLBACK run_library(HWND hwnd, HINSTANCE hinst, LPSTR lpsz_cmd_line, int n_cmd_show)
{
    std::istringstream iss(lpsz_cmd_line);
    int rows = 0, cols = 0, threads_n = 0, max_concurrent_threads = 1;

    if (!(iss >> rows >> cols >> threads_n >> max_concurrent_threads)) 
    {
        MessageBoxW(NULL, L"Wrong params!", L"Error", MB_OK);
        return;
    }
...
extern"C" __declspec(dllexport) void run_calculation(int rows, int cols, int threads_n, int max_concurrent_threads)
{
    const std::string m_file = "D:\\Programming\\C,C++Applications\\OSAOP\\Lab_4\\matrix.txt";
    const std::string r_file = "D:\\Programming\\C,C++Applications\\OSAOP\\Lab_4\\result.txt";
