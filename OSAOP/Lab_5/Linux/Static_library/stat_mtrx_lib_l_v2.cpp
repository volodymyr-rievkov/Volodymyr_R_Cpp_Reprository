#include"stat_mtrx_lib_l_v2.hpp"
...
void generate_matrix_into_file(const std::string& file_name, const size_t rows, const size_t cols)
{
...
matrix read_matrix_from_file(const std::string& file_name, const size_t rows, const size_t cols) 
{
    matrix m(rows, std::vector<int>(cols));

    int file_h = open(file_name.c_str(), O_RDONLY);
    if (file_h == -1) 
    {
        show_message("Error: File cannot be opened!");
        return m;
    ...
    if (fstat(file_h, &file_stat) == -1) 
    {
        show_message("Error: Could not get file size!");
        close(file_h);
        return m;
    }
    #ifdef __linux__
        char* file_data_p = static_cast<char*>(mmap(NULL, file_stat.st_size, PROT_READ, MAP_PRIVATE, file_h, 0));
        if (file_data_p == MAP_FAILED) 
        {
            show_message("Error: Could not map view of file!");
            close(file_h);
            return m;
        }

        std::string data(file_data_p, file_stat.st_size);
        ...
        for (size_t i = 0; i < rows; ++i) 
        {
            for (size_t j = 0; j < cols; ++j) 
            {
                if (!(stream >> m[i][j])) 
                {
                    show_message("Error: Failed to read matrix data!");
                    munmap(file_data_p, file_stat.st_size);
                    close(file_h);
                    return m;
                }
            }
...
static std::string matrix_to_string(const matrix& m) 
{
    std::stringstream ss;
...
{
    std::string matrix_text = matrix_to_string(m);

    int file_h = open(file_name.c_str(), O_RDWR | O_CREAT | O_TRUNC, 0666);
    if (file_h == -1) 
    {
        show_message("Error: File cannot be opened!");
        return;
    ...
    size_t matrix_size = matrix_text.size();
    
    if (ftruncate(file_h, matrix_size) == -1) 
    {
        show_message("Error: Could not set file size!");
        close(file_h);
        return;
    }
    #ifdef __linux__
        char* file_data_p = static_cast<char*>(mmap(NULL, matrix_size, PROT_READ | PROT_WRITE, MAP_SHARED, file_h, 0));
        if (file_data_p == MAP_FAILED) 
        {
            show_message("Error: Could not map view of file!");
            close(file_h);
        ...
        memcpy(file_data_p, matrix_text.c_str(), matrix_size);

        munmap(file_data_p, matrix_size);
        close(file_h);
    #endif
}
