#include"stat_mtrx_lib_w_v2.hpp"
...
matrix read_matrix_from_file(const std::string& file_name, const size_t rows, const size_t cols) 
{
    ...
    {
        CloseHandle(file_h);
        MessageBoxW(NULL, L"Error: Could not create file mapping!", L"Error", MB_OK | MB_ICONERROR);
        return m; 
    }

    LPVOID file_data_p = MapViewOfFile(
        file_map_h,                       
        FILE_MAP_READ,                   
        0,                              
        0,                               
    ...
    if (file_data_p == NULL) 
    {
        ...
        CloseHandle(file_h);
        MessageBoxW(NULL, L"Error: Could not map view of file!", L"Error", MB_OK | MB_ICONERROR);
        return m; 
    }

    char* char_buffer = static_cast<char*>(file_data_p);
    std::string data(char_buffer);  
    ...
    for (size_t i = 0; i < rows; ++i) 
    {
        for (size_t j = 0; j < cols; ++j) 
        {
            if (!(stream >> m[i][j])) 
            {
                MessageBoxW(NULL, L"Error: Failed to read matrix data!", L"Error", MB_OK | MB_ICONERROR);
                return m; 
            }
        }
    }

    UnmapViewOfFile(file_data_p);
    CloseHandle(file_map_h);
    CloseHandle(file_h);
...
void write_matrix_to_file(const matrix& m, const std::string& file_name) 
{
    ...
    memcpy(file_data_p, matrix_text.c_str(), matrix_text.size());
